...
@section('content')
    ...
    function timeAgo($date)
    {
        ...
        if ($elapsed >= 31536000) {
            $interval = floor($elapsed / 31536000);
            $unit = $interval > 1 ? 'years' : 'year';
        } elseif ($elapsed >= 2592000) {
            $interval = floor($elapsed / 2592000);
            $unit = $interval > 1 ? 'months' : 'month';
        } elseif ($elapsed >= 604800) {
            $interval = floor($elapsed / 604800);
            $unit = $interval > 1 ? 'weeks' : 'week';
        } elseif ($elapsed >= 86400) {
            $interval = floor($elapsed / 86400);
            $unit = $interval > 1 ? 'days' : 'day';
        } elseif ($elapsed >= 3600) {
            $interval = floor($elapsed / 3600);
            $unit = $interval > 1 ? 'hours' : 'hour';
        } elseif ($elapsed >= 60) {
            $interval = floor($elapsed / 60);
            $unit = $interval > 1 ? 'minutes' : 'minute';
        } else {
            return 'just now';
        }
